...
def data_dict_to_sents(data_dict):
    ...
    for meme_df in data_dict.values():
        for sentence in list(tuple(meme_df.groupby('meme_id'))):
            sentences.append(list(sentence[1]['token']))
    return sentences
...
def create_model(meta_dict):
	...
	model.add(LSTM(units = len(meta_dict["meme_names"]), recurrent_dropout = .5, input_shape = (meta_dict["sentence_size"], meta_dict["embeddings"].vector_size + len(meta_dict["sentiment"]) + len(meta_dict["pos"]))))
