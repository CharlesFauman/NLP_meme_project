...
class Application(): #the main class that we call "Game"
	def __init__(self): #setting up the window for the game here
		...
		self.canvas = Canvas(master = self.frame, width=1200, height=750, bg = "white", highlightthickness = 0) #actually creates a window and puts our frame on it
...
class ImageHolder():
	def __init__(self, application):
		...
		for imagefile in glob.glob("../collection/templates/*.jpg"): #assuming jpg
			...
			resized_image = ImageOps.fit(image, (self.image_size,self.image_size), Image.ANTIALIAS)
	...
	def paint(self):
		...
		for index, meme in enumerate(self.images):
		
			y = (index - (index%8))/8
			self.application.canvas.create_image((index%8)*self.image_size, self.image_size*y, image = self.images[analysis_meta_dict["meme_names"][index]], anchor = 'nw')
			self.application.canvas.create_rectangle((index%8)*self.image_size, self.image_size*y, (index%8)*self.image_size+30, self.image_size*y+30, fill = 'black')
			self.application.canvas.create_text((index%8)*self.image_size, self.image_size*y, text = index, fill = 'white', font="Times 20 bold", anchor = 'nw')
